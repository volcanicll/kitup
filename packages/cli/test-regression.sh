...
entry_output="$(PATH="$TEST_PATH" HOME="$TMP_DIR/home" bash "$ROOT_DIR/kitup" --version)"
...
exit 1
'
PATH="$TMP_DIR/npm-global/bin:$TEST_PATH" HOME="$TMP_DIR/home" bash "$ROOT_DIR/kitup.sh" cline -n > "$TMP_DIR/cline.out"
...
PATH="$TEST_PATH" HOME="$TMP_DIR/home" bash "$ROOT_DIR/kitup.sh" kimi --install -n > "$TMP_DIR/kimi.out"
...
exit 1
'
PATH="$TEST_PATH" HOME="$TMP_DIR/home" bash "$ROOT_DIR/kitup.sh" qwen -n > "$TMP_DIR/qwen.out"
