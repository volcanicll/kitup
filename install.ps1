#
# AI Tools Updater - Windows Installation Script
# One-click installer for the AI coding tools updater
# Usage: irm https://raw.githubusercontent.com/user/update-ai-tools/main/install.ps1 | iex
#

$ErrorActionPreference = "Stop"

# Configuration
$RepoOwner = $env:REPO_OWNER -or "yourusername"
$RepoName = $env:REPO_NAME -or "update-ai-tools"
$Version = $env:VERSION -or "main"
$InstallDir = $env:INSTALL_DIR -or "$env:LOCALAPPDATA\update-ai-tools"
...
function Install-Updater {
    ...
    $scriptUrl = "https://raw.githubusercontent.com/$RepoOwner/$RepoName/$Version/update-ai-tools.ps1"
    $scriptPath = Join-Path $InstallDir "update-ai-tools.ps1"
    $wrapperPath = Join-Path $InstallDir "update-ai-tools.bat"

    Write-Info "Downloading update-ai-tools..."
    ...
    Write-Success "Downloaded update-ai-tools.ps1 to $scriptPath"
    ...
    Write-Host ""
    Write-Info "Usage:"
    Write-Host "  update-ai-tools --help       Show help information"
    Write-Host "  update-ai-tools --status     Check installed AI tools status"
    Write-Host "  update-ai-tools --all        Update all installed AI tools"
    Write-Host ""
    Write-Info "To get started, run: update-ai-tools --status"
}

# Uninstall function
function Uninstall-Updater {
    Write-Info "Uninstalling AI Tools Updater..."
...
# Show help
function Show-Help {
    Write-Host "AI Tools Updater - Installer"
    Write-Host ""
    Write-Host "Usage:"
    ...
    Write-Host "  REPO_NAME     GitHub repository name (default: update-ai-tools)"
    Write-Host "  VERSION       Version to install (default: main)"
    Write-Host "  INSTALL_DIR   Installation directory (default: %LOCALAPPDATA%\update-ai-tools)"
    Write-Host ""
    Write-Host "Options:"
    Write-Host "  --help        Show this help message"
    Write-Host "  --uninstall   Uninstall update-ai-tools"
...
if ($args -contains "--version" -or $args -contains "-v") {
    Write-Host "AI Tools Updater Installer v1.0.0"
    exit 0
}
